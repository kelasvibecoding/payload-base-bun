...
import { staggeredContainerVariants, staggeredItemVariants } from '@/features/app-shell/components/motion-config'

function FeatureCard({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <m.div variants={staggeredItemVariants}>
      <GlassCard className="h-full max-w-none p-6 hover:border-primary/30 transition-colors">
...
export function HomeFeatures() {
  return (
    <section className="relative z-10 py-32 px-4 font-sans">
      <div id="features" className="absolute -top-[100px] left-0 h-px w-full opacity-0 pointer-events-none" />
