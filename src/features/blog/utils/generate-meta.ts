...
export const generateMeta = async (args: { doc?: Post }): Promise<Metadata> => {
  ...
  if (!doc) {
    return {
      ...
      authors: [{ name: 'Payload Team' }],
      creator: 'Payload Team',
      publisher: siteName,
      robots: {
        index: true,
        follow: true,
  ...
  const image = (doc.meta as any)?.image
  ...
  const url = `${siteUrl}/blog/${doc.slug}`
      ...
      ...(locales.length > 1 && {
        languages: locales.reduce(
          (acc, loc) => {
            ...
            acc[localeCode] = `${siteUrl}/blog/${doc.slug}`
            return acc
          },
