...
> **Important**: When your access control functions are executed via the Access Operation, the `id`, `data`, `siblingData`, `blockData` and `doc` arguments will be **undefined**. Additionally, Where queries returned from access control functions will not be run - we'll assume the user does not have access instead.
