...
3.  **Run**: `pnpm generate:types` to regenerate TypeScript interfaces.
...
### The `devsafe` Command
**Command**: `pnpm devsafe`
**Purpose**: Next.js development cache can sometimes get corrupted or stale.
**Workflow**:
- Always start your day with `pnpm devsafe`.
- If you see weird styling or stale data, stop the server and run `pnpm devsafe`.
...
- **Fix**: Run `pnpm generate:importmap` or restart with `pnpm devsafe`.
...
2.  **Build**: Run `pnpm build` locally to verify no build errors.
